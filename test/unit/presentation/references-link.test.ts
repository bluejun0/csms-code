...
import {
  SHOW_STRING_REFERENCES_COMMAND, SHOW_CONFIG_REFERENCES_COMMAND, ShowReferencesArgs,
  commandForKind, lensTitle, referencesLinkMarkdown,
} from '../../../src/presentation/references-link';
...
describe('commandForKind — 대상 종류별 명령', () => {
  it('string → showStringReferences, config → showConfigReferences', () => {
...
describe('referencesLinkMarkdown — hover 아래 명령 링크', () => {
  it('색인 후: "사용 N건 보기" 링크', () => {
    const md = referencesLinkMarkdown(SHOW_STRING_REFERENCES_COMMAND, ARGS, 2);
    assert.ok(md.startsWith(`[사용 2건 보기](command:${SHOW_STRING_REFERENCES_COMMAND}?`), md);
  });
  it('색인 전: "사용 찾기" 링크', () => {
    assert.ok(referencesLinkMarkdown(SHOW_CONFIG_REFERENCES_COMMAND, ARGS, null).startsWith(`[사용 찾기](command:${SHOW_CONFIG_REFERENCES_COMMAND}?`));
  });
