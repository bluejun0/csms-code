...
- **`install.xml`의 `<TABLE>` 줄에서 그 테이블의 사용처를 찾습니다.** Shift+F12로 SQL 문자열의 `{tablename}`과 `$DB->insert_record('tablename', …)` 같은 DML 호출을 모두 보고, 선언 줄 위의 **"사용 N건" 버튼**으로도 같은 목록을 엽니다("모든 참조 찾기"에는 선언 위치도 함께). 설정 `csmscode.tables.codeLens`(기본 `true`).
  - 실측(hlulxp): `install.xml` 125개에 `<TABLE>` 선언 713개, 그중 **670개(94%)가 실제로 참조**됩니다. 색인에 담긴 테이블 사용처는 SQL `{name}` 13,828건 + `$DB` 리터럴 인자 19,144건입니다(`sql_like` 같은 SQL 조각 헬퍼는 첫 인자가 컬럼이라 제외).
  - `{name}`은 실재 테이블인지 걸러내지 않고 담습니다 — 걸러 두면 `install.xml`에 테이블을 새로 추가했을 때 그 테이블이 "사용 0건"으로 보이기 때문입니다. 조회는 색인된 테이블 이름으로만 하므로 `index.php?id={course}` 같은 비SQL 중괄호는 목록에 영향을 주지 않습니다.
...
- 사용처 색인에 테이블 사용처가 더해져 **항목이 55,568 → 88,486건(+59%)**이 됩니다. 실측 영향(hlulxp, 웜):
  ...
  - 스캔 없이 불러오는 이득은 그대로입니다(로드가 스캔의 1/10 이하).
