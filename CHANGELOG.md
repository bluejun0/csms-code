...
### 변경
- **사용처 색인 스캔이 두 배 빨라졌습니다.** 디렉터리마다 부르던 `realpath`를 심볼릭 링크로 진입할 때만 부르고(순환 가드는 그대로), 하위 디렉터리 열거와 파일 읽기를 병렬로 합니다. 실측(hlulxp, 색인 대상 24,717개): 전체 빌드 **웜 8.5~11.2초 → 4.5초**, 콜드 53.5초 → 35.5초. 항목·위치·순서는 전과 동일합니다(적용은 파일 순서대로).
...
- **사용처 색인을 디스크에 저장합니다.** 다음에 그 워크스페이스를 열면 스캔 없이 **0.1초**에 불러와 Shift+F12와 "사용 N건" 버튼이 바로 동작합니다. 불러온 뒤에는 백그라운드에서 파일 mtime·크기만 비교해(읽지 않습니다) 바뀐 파일만 다시 읽고, 반영되면 버튼·하이라이트를 다시 그립니다. 저장·삭제 뒤에도 캐시를 갱신합니다.
  - 실측(hlulxp): 전체 스캔 4,422ms → 캐시 로드 **105ms**(읽기·검증 24ms + 복원 81ms), 캐시 쓰기 176ms·0.90MB, 변경 없을 때 백그라운드 검증 1,119ms. 복원한 색인이 스캔한 색인과 같은지 (컴포넌트,키) 22,964쌍 전수 비교에서 불일치 0건.
