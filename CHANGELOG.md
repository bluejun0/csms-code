...
- **lang 파일에 "사용처 N곳" 버튼(CodeLens).** `$string['key']` 줄 위에 버튼이 붙고, 클릭하면 그 자리에서 사용처 목록(peek)이 열립니다. 사용처 색인이 아직 없으면 "사용처 보기"로 표시되고 클릭이 색인을 만듭니다 — 파일을 여는 것만으로는 워크스페이스를 스캔하지 않습니다. 개수는 화면에 보이는 줄에만 계산합니다. 설정 `csmscode.strings.codeLens`(기본 `true`).
- **hover 아래 "사용처 N곳 보기" 링크.** 코드 쪽 문자열 hover(PHP·JS·mustache) 맨 아래에서 클릭하면 같은 목록이 열립니다. 링크가 부르는 명령 하나만 신뢰하므로 lang 값 텍스트 안의 다른 링크는 실행되지 않습니다.
