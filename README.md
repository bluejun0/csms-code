...
- **7. 플러그인 탐색기**
  ...
  ▸ 템플릿      → .mustache 파일로 이동
  ▸ 설정        → settings.php의 admin_setting 선언 줄로 이동
  ```

  **설정 뷰는 처음 펼칠 때 색인을 만듭니다**(몇 백 ms~수 초). 열지 않으면 아무 비용도 들지 않습니다.
...
- **8. 찾기**
  ...
  각 뷰 제목줄의 **돋보기 버튼**을 누르면 그 카테고리만 찾습니다.
  전체를 한 번에 찾으려면 `Ctrl + Shift + P` → `CSMS Code: 찾기`.
