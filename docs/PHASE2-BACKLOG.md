...
- **AMD 모듈 참조의 미해석 요인**: 실측 해석률은 추출된 338건 중 328건(97.0%) — 정규식으로 센 전체 호출 354건 기준으로는 약 93%다(차이 16건은 겹따옴표 2건 + 주석 처리된 호출). 남는 원인은 ① `PLUGIN_DIRS`에 없는 플러그인 타입(`gradingform`→`grade/grading/form`, `assignfeedback`→`mod/assign/feedback`, `quizaccess`→`mod/quiz/accessrule`, `quiz`(리포트)→`mod/quiz/report`) ② 실제로 없는 모듈(`local_ubion/assign`·`mod_ubboard/ubboard` — 죽은 참조) ③ 겹따옴표 리터럴(실측 2건) ④ `lib/components.json`이 없는 구버전(3.5·2.9)에서 코어 서브시스템 모듈. ①은 아래 19번으로 한 번에 해소된다.
...
19. **`lib/components.json`으로 `PLUGIN_DIRS`를 대체**: 이 파일의 `plugintypes`(4.5 기준 45개)가 타입→디렉터리 매핑의 권위 있는 출처다. 정적 `PLUGIN_DIRS`(30개)를 이것으로 대체하면 **6번 잔여(qbank·tiny·quizaccess·assignsubmission 등)와 AMD 미해석 ①이 한 번에 해소되고**, 새 Moodle 버전에서 타입이 늘어도 코드 수정이 필요 없다. 파일이 없는 구버전(3.5·2.9)에는 정적 맵을 폴백으로 남긴다. 네 색인(install.xml·lang·템플릿·AMD)이 모두 같은 맵을 쓰므로 한 곳만 바꾸면 되지만, 열거 대상이 늘어 색인 크기·시간이 변하므로 실측을 함께 해야 한다. **함께 처리할 것**: `coreSubsystemDirs`가 호출마다 `readFileSync` + `JSON.parse`를 한다(`componentOfAmdFile` 경로 — 워처 이벤트·Shift+F12마다). `components.json`은 코어 업그레이드 때만 바뀌므로 루트별 메모이즈가 안전하고, 이 항목에서 파일을 더 자주 읽게 되므로 그때 함께 넣는다.
