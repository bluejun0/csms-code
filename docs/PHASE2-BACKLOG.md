...
- **동적 컴포넌트 호출은 역참조에 안 나온다**: `get_string('k', $comp)`는 정의 이동·hover·하이라이트·진단이 되지만, lang 파일의 Shift+F12 목록에는 없다 — 사용처 색인(`php-usage-index.ts`)이 정규식 기반이라 리터럴 컴포넌트만 본다. 정의 이동은 되는데 역참조는 안 되는 비대칭이라 사용자가 버그로 읽는다. 해결하려면 사용처 색인도 팩트 기반으로 바꿔야 해서 별도 사이클이다.
