...
- **사용처 색인의 메모리·캐시 수명**: 색인 하나가 힙 약 100MB(항목 55,568건 실측)이고 창(확장 호스트)마다 별개다. 위치를 typed array로 바꾸는 다이어트가 후속 후보. 캐시 파일은 워크스페이스당 약 1MB가 전역 저장소에 쌓이고 스스로 지워지지 않으며(수명 관리 미구현), rename 전에 프로세스가 죽으면 `.tmp`가 남는다. 순회 팬아웃에도 상한이 없다(24,717개에서는 실측 문제없음).
...
16. **테이블 표면의 남은 조각들** (2026-08-06 SQL 테이블 이동에서 의도적으로 제외 — 설계: `docs/superpowers/specs/2026-08-06-sql-table-navigation-design.md`):
    ...
    - ~~**install.xml → 사용처 참조(Shift+F12)**~~ — ✅ 완료 (2026-09-02, 0.21.0). `<TABLE>` 줄에서 Shift+F12·"사용 N건" 버튼. 사용처 색인에 테이블 종류를 더해 항목이 +50%(메모리 창당 100→150MB), 스냅샷 형식 v2. 컬럼(`<FIELD>`)별 사용처는 범위 밖.
