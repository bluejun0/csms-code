...
- **AMD 모듈 참조의 미해석 요인**: 실측 해석률은 추출된 338건 중 328건(97.0%) — 정규식으로 센 전체 호출 354건 기준으로는 약 93%다(차이 16건은 겹따옴표 2건 + 주석 처리된 호출). 남는 원인은 ~~① 미매핑 플러그인 타입~~(2026-08-07 타입 맵 전환으로 해소) ② 실제로 없는 모듈(`local_ubion/assign`·`mod_ubboard/ubboard` — 죽은 참조) ③ ~~겹따옴표 리터럴~~(인용 형태를 가리지 않게 되어 해소) ④ `lib/components.json`이 없는 구버전(3.5·2.9)에서 코어 서브시스템 모듈. 
- **코어 클래스 색인은 세션 중 갱신되지 않는다**: 첫 요청에 한 번 만들고 전체 재빌드에서도 다시 만들지 않는다(코어 클래스는 편집 대상이 아니라 문제되지 않는다). 설정 키 색인은 0.17.0부터 `settings.php`·`admin/settings/*.php` 저장 시 그 파일만 증분 갱신되어 `get_config` 탐색과 `$CFG->` 완성에 바로 반영된다. 타입 맵의 `coreSubsystemDirs` 메모이즈 항목과 같은 자리에서 함께 처리할 수 있다.
- **동적 컴포넌트 호출은 사용처 목록·개수에 안 나온다**: `get_string('k', $comp)`는 정의 이동·hover·하이라이트·진단이 되지만, Shift+F12 목록(코드 쪽·lang 쪽 모두)과 lang CodeLens의 "사용 N건" 개수에는 없다 — 사용처 색인(`php-usage-index.ts`)이 정규식 기반이라 리터럴 컴포넌트만 본다. 2026-08-25에 코드 쪽 참조·버튼·링크가 붙어 **진입점** 비대칭은 해소됐고, 남은 것은 색인 **커버리지**다. 해결하려면 사용처 색인도 팩트 기반으로 바꿔야 해서 별도 사이클이다(실측 동적 호출 2,406건 — 그중 2,081건은 전파로 해석되므로 색인이 팩트를 쓰면 그만큼이 목록에 들어온다).
- **사용처 색인의 메모리·캐시 수명**: 색인 하나가 힙 약 123MB(항목 88,486건 실측 — 0.21.0에서 테이블 종류 추가)이고 창(확장 호스트)마다 별개다. 위치를 typed array로 바꾸는 다이어트가 후속 후보이고, 더 값싼 레버는 **이름 문자열 인터닝**이다(정규식 캡처가 매치마다 새 문자열을 만든다 — 테이블 이름 32,972개를 약 1,400개로 접을 수 있다). 캐시 로드가 항목 +59%에 3~8배로 늘어난 것도 `unpackRows`의 항목당 객체 생성이 의심되어 같은 자리에서 볼 값어치가 있다. 캐시 파일은 워크스페이스당 약 1MB가 전역 저장소에 쌓이고 스스로 지워지지 않으며(수명 관리 미구현), rename 전에 프로세스가 죽으면 `.tmp`가 남는다. 순회 팬아웃에도 상한이 없다(24,717개에서는 실측 문제없음).
- **settings.php 선언 파서의 한계**: 정규식 순차 스캔이라 함수 스코프를 모른다 — 서브클래스 생성자의 `parent::__construct($name, …)`처럼 값이 매개변수인 변수는 침묵하고, 관용구(`$v = 'p/k'`·`$v = $u . '/k'`·`"$u/k"`) 밖의 대입은 값을 지운다. 주석 처리된 선언은 무시한다. 함수 반환값·배열에서 오는 이름은 잡지 않는다.
- **컴포넌트 전파의 사각**: 문자열 보간(`"block_{$b->name}"`)·연결(`'mod_' . $type`)·함수 반환값에서 온 컴포넌트는 해석하지 않는다(실측 동적 2,406건 중 325건). 키 유일성 폴백(색인 전체에서 그 키를 가진 컴포넌트가 하나면 그리로 — 실측 2,070건 해당)이 후보지만 근거가 추측이라 이동·hover에만 쓸 수 있고 진단에는 못 쓴다.
...
15. ~~**알려진 전역의 바인딩**~~ — ✅ 완료 (2026-08-07, 설계: `docs/superpowers/specs/2026-08-07-moodle-globals-design.md`). `$USER`·`$COURSE`·`$SITE`는 테이블 컬럼으로, `$DB`·`$PAGE`·`$OUTPUT`은 코어 클래스 멤버로, `$CFG`는 설정 키로 해석한다. 진단은 비목표(런타임 필드·키 때문). **남은 것**: `get_course()`·`get_coursemodule_from_id()` 같은 함수 반환값 바인딩(실측 45회)은 여전히 미지원.
16. **테이블 표면의 남은 조각들** (2026-08-06 SQL 테이블 이동에서 의도적으로 제외 — 설계: `docs/superpowers/specs/2026-08-06-sql-table-navigation-design.md`):
    ...
    - ~~**`$DB` 메서드 인자 리터럴 이동**~~ — ✅ 완료 (2026-08-11). SQL의 `{table}`과 같은 팩트에 담아 정의 이동·하이라이트가 한 경로를 탄다. 실측 리터럴 첫 인자 1,150건 중 91.3% 해석(나머지는 컬럼 이름이거나 색인에 없는 테이블). `sql_*` 헬퍼는 첫 인자가 컬럼·식이라 제외한다 — 실측 `block_rss_client`·`feedback` 두 건이 테이블 이름과 겹쳐 잘못 링크될 뻔했다.
    - ~~**install.xml → 사용처 참조(Shift+F12)**~~ — ✅ 완료 (2026-09-02, 0.21.0). `<TABLE>` 줄에서 Shift+F12·"사용 N건" 버튼. 사용처 색인에 테이블 종류를 더해 항목이 55,568 → 88,486(+59%), 메모리 창당 100 → 123MB, 캐시 0.90 → 1.09MB·로드 0.11 → 0.3~0.9초, 스냅샷 형식 v2. 컬럼(`<FIELD>`)별 사용처는 범위 밖.
...
- ~~**플러그인 설정 키(get_config) 탐색**~~ — ✅ 완료 (2026-08-26, 0.17.0, 설계: `docs/superpowers/specs/2026-08-26-config-key-navigation-design.md`). settings.php 선언 파서(순차 대입 관용구)·(plugin, key) 선언 색인·팩트·사용처·정의/hover/하이라이트/양방향 참조/CodeLens/완성. 실측 해석 89.7%. **후속 후보**: B 코어 키(`$CFG->x`·`get_config('core')` — 사용처 색인 18,894건 부담), C 통째 접근(`get_config('p')->key` 72건 — 변수 추적), 누락 키 진단("선언됐거나 어디선가 set_config된 키"를 존재로 보는 규칙 검토), hover 제목(`$title = get_string(…)` 한 단계 더 추적)·기본값, `set_config` 값에 괄호가 든 호출의 사용처(정규식 한계).
- ~~**문자열 호출 가족 확장**~~ — ✅ 완료 (2026-08-26, 0.16.0, bounded). `print_error`·`new moodle_exception`(`\` 접두 포함)·`new lang_string`·`new help_icon` + 컴포넌트 생략 한 인자 호출(core/error 기본). 형태 표 `string-functions.ts` 하나를 tree-sitter(쿼리 4개)·사용처 정규식·완성 트리거가 공유. 실측 hlulxp 커스텀: 한 인자 `moodle_exception`/`print_error` 172건 중 118건이 `error.php`에 없음(런타임 `[[code]]`). **잔여(비목표)**: 키가 2번째 인자인 `->addHelpButton($el, 'k', 'c')`(커스텀 20·코어 911 — `k`와 `k_help` 두 문자열), `->help_icon()`·`get_string_manager()->get_string/string_exists` 메서드 호출(11), `get_strings([...])`·JS `getStrings`, `new` 꼴의 동적 컴포넌트(10). capability(`has_capability('local/x:view')` 91·`require_capability` 46)는 `db/access.php` 정의로 이어지는 별도 사이클.
- ~~**코드 쪽 문자열 참조 + 사용처 버튼·링크 + `print_string`**~~ — ✅ 완료 (2026-08-25, bounded — 설계는 채팅에서 확정). 코드(PHP·JS·mustache)의 키 위에서 Shift+F12, lang 파일 `$string` 줄의 "사용 N건" CodeLens, hover 아래 "사용 N건 보기" 링크, `print_string` 인식, `$string["key"]` 역참조 수정. 비목표: 동적 컴포넌트 사용처 색인(위 알려진 제한), 템플릿·AMD 파일 CodeLens, `lang_string`·`get_strings`. 후속 후보: `LangReferenceProvider`의 키 인식(`langKeyAt` — 한 줄 첫 `$string[`만, 이스케이프된 인용부호 키 미인식)을 CodeLens처럼 `parseLangFile` 항목(줄 번호 일치)에서 찾게 해 F12·Shift+F12·버튼의 키 인식을 한 소스로 합치기.
