...
- **사용처 색인의 메모리·캐시 수명**: 색인 하나가 힙 약 100MB(항목 55,568건 실측)이고 창(확장 호스트)마다 별개다. 위치를 typed array로 바꾸는 다이어트가 후속 후보. 캐시 파일은 워크스페이스당 약 1MB가 전역 저장소에 쌓이고 스스로 지워지지 않는다(수명 관리 미구현).
