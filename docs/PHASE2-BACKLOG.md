...
- **동적 컴포넌트 호출은 사용처 목록·개수에 안 나온다**: `get_string('k', $comp)`는 정의 이동·hover·하이라이트·진단이 되지만, Shift+F12 목록(코드 쪽·lang 쪽 모두)과 lang CodeLens의 "사용처 N곳" 개수에는 없다 — 사용처 색인(`php-usage-index.ts`)이 정규식 기반이라 리터럴 컴포넌트만 본다. 2026-08-25에 코드 쪽 참조·버튼·링크가 붙어 **진입점** 비대칭은 해소됐고, 남은 것은 색인 **커버리지**다. 해결하려면 사용처 색인도 팩트 기반으로 바꿔야 해서 별도 사이클이다(실측 동적 호출 2,406건 — 그중 2,081건은 전파로 해석되므로 색인이 팩트를 쓰면 그만큼이 목록에 들어온다).
...
- ~~**코드 쪽 문자열 참조 + 사용처 버튼·링크 + `print_string`**~~ — ✅ 완료 (2026-08-25, bounded — 설계는 채팅에서 확정). 코드(PHP·JS·mustache)의 키 위에서 Shift+F12, lang 파일 `$string` 줄의 "사용처 N곳" CodeLens, hover 아래 "사용처 N곳 보기" 링크, `print_string` 인식, `$string["key"]` 역참조 수정. 비목표: 동적 컴포넌트 사용처 색인(위 알려진 제한), 템플릿·AMD 파일 CodeLens, `lang_string`·`get_strings`. 후속 후보: `LangReferenceProvider`의 키 인식(`langKeyAt` — 한 줄 첫 `$string[`만, 이스케이프된 인용부호 키 미인식)을 CodeLens처럼 `parseLangFile` 항목(줄 번호 일치)에서 찾게 해 F12·Shift+F12·버튼의 키 인식을 한 소스로 합치기.
