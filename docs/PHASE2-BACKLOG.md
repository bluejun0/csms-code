...
- **전역 색인은 세션 중 갱신되지 않는다**: 코어 클래스 색인은 첫 요청에 한 번 만들고 전체 재빌드에서도 다시 만들지 않는다(설정 키 색인은 0.17.0부터 `settings.php`·`admin/settings/*.php` 저장 시 그 파일만 증분 갱신된다). 플러그인 `settings.php`에 설정을 추가하면 창을 다시 열기 전까지 `$CFG->` 완성에 나타나지 않는다(코어 클래스는 편집 대상이 아니라 문제되지 않는다). 타입 맵의 `coreSubsystemDirs` 메모이즈 항목과 같은 자리에서 함께 처리할 수 있다.
