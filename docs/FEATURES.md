...
이동하지 않습니다. 실제로 존재하지 않는 모듈과 `lib/components.json`이 없는 구버전의 코어
서브시스템 모듈은 해석되지 않고 조용히 아무 일도 하지 않습니다.
...
**인용 형태** — 참조를 읽는 모든 자리에서 홑따옴표와 겹따옴표를 같게 봅니다. PHP에서 보간이 없는
겹따옴표는 홑따옴표와 같은 값이기 때문입니다. 보간이 섞인 문자열(`"local_{$type}/card"`)은
리터럴이 아니므로 조각을 참조로 오인하지 않고 침묵합니다.
...
| 명령 | 하는 일 |
|---|---|
| `CSMS Code: 찾기` | 테이블·문자열·API·템플릿·설정을 이름과 한국어 값·설명으로 한 번에 검색합니다(§17). |
...
왼쪽 액티비티 바의 CSMS 아이콘 → **테이블·문자열·API·템플릿·설정** 뷰 다섯. 뷰 하나가 카테고리
하나이고, 각 뷰는 `컴포넌트 → 항목` 두 단계입니다. "테이블만 보기"가 뷰를 고르는 일이 됩니다.
뷰 제목줄의 **돋보기 버튼**은 그 카테고리만 찾습니다(§17).
...
| 설정 | 설정 키(키순) | 설정 종류(`configtext` 등) | `settings.php`의 선언 줄 |

**설정 뷰만 색인이 지연 생성입니다** — 처음 펼칠 때 만듭니다(실측 웜 325~440ms, 콜드 최대 19초).
활성화에 붙이면 워크스페이스를 열 때마다 그만큼 늦어지고, 뷰를 안 여는 사람에게는 순전한 낭비입니다.
다 만들어지면 뷰가 스스로 다시 그립니다.

**설정 이름은 한국어로 보여주지 않습니다.** `admin_setting_configtext('local_x/key', get_string('key', 'local_x'), …)`
관례상 설정 키와 lang 키가 같을 것 같지만, 실측 603개 중 14개만 일치합니다.

**그 카테고리에 항목이 있는 컴포넌트만 나옵니다** — 컴포넌트 이름 옆에 항목 수가 붙습니다.
실측(전체 591개 중): 테이블 124, 문자열 585, API 72, 템플릿 183. 뷰가 곧 카테고리이므로
테이블 뷰에 테이블 없는 플러그인 467개를 늘어놓지 않습니다. "비었는데 아직 색인 중인가"는
상태 표시줄이 답합니다(§13).
...
**개수는 항목을 실제로 만들어 셉니다.** 실측(컴포넌트 591개) 목록 만들기는 테이블 6ms,
문자열 13ms, API 2ms, 템플릿 23ms입니다 — 따로 싼 카운트 경로를 두면 표시와 개수가 어긋날
위험만 생깁니다. 컴포넌트를 펼치기 전에는 그 컴포넌트의 항목을 만들지 않습니다.
...
네 카테고리를 가로지르는 검색입니다. **트리의 기본 찾기는 이미 펼친 노드만 훑습니다** —
컴포넌트가 접혀 있으면 그 안의 항목은 만들어지지도 않으므로(§16의 지연 로딩) 별도 표면이
필요합니다.
...
훑는 텍스트를 표시용 부제와 나눕니다 — 테이블 부제가 `컬럼 12`라 `12`로 검색했을 때 엉뚱하게
걸리면 안 됩니다.

**순위** — ①이름이 검색어로 시작 ②이름에 포함 ③값·설명에만 포함. 같은 순위에서는 커스텀
플러그인이 코어보다 앞이고, 그다음 컴포넌트·이름순입니다. 상한 200건.

**모든 항목에 `alwaysShow`를 겁니다** — 걸러내기와 순위를 우리가 정하기 때문입니다. VSCode
기본 필터에 맡기면 한국어 값으로만 걸린 항목을 떨어뜨리고 제 점수로 순위를 다시 매깁니다.

**빈 질의는 아무것도 주지 않습니다** — 4만 건을 늘어놓을 이유가 없습니다.

**뷰 제목줄의 돋보기는 그 카테고리만 찾습니다.** `view/title` 메뉴는 명령에 인자를 넘기지 못하므로
카테고리마다 명령을 따로 두고(`csmscode.search.tables` 등) 명령 팔레트에서는 숨깁니다 —
팔레트에는 전체를 훑는 `CSMS Code: 찾기` 하나만 남습니다.

**전체 찾기는 설정 색인이 이미 있을 때만 설정을 훑습니다.** 검색어 한 글자에 색인 생성(최대 19초)을
끼워 넣지 않습니다. 설정 뷰의 돋보기는 색인을 먼저 만들고 엽니다.

**평면 목록은 찾기를 여는 동안만 들고 있습니다.** 첫 질의에 만들고, 창을 닫거나 색인이 바뀌면
버립니다. 실측(`dghulxp`, 검색 대상 44,692건): 만드는 데 110ms, 질의마다 3~8ms.
상주시키면 쓰지 않는 세션에도 수 MB가 남습니다(§0.23.0의 메모리 작업과 같은 이유).
