...
84. `Ctrl+Shift+P` → `CSMS Code: 찾기` → `출석부` 입력 → 그 값을 가진 문자열 키들이 나온다(키 이름에는 "출석부"가 없는데도). 엔터 → `lang/ko`의 그 `$string` 줄로 이동
85. `자막` 입력 → 문자열과 **API가 섞여** 나오고, 각 줄 앞에 카테고리 아이콘이 다르다. API 줄은 `read · 강좌 자막 리스트 · local_ubllmapi`처럼 보인다
86. `attendance` 입력 → 이름이 `attendance`로 **시작하는** 것이 먼저, 가운데 포함된 것이 그다음, 한국어 값에만 있는 것이 마지막이다. 커스텀 플러그인이 `core_*`보다 위에 있다
87. `컬럼` 입력 → 테이블이 걸리지 **않는다**(부제의 `컬럼 12`는 검색 대상이 아니다)
88. 입력을 지워 빈 칸으로 두면 목록이 비어 있다(4만 건이 쏟아지지 않는다)
89. 창을 처음 열고 첫 글자를 칠 때 잠깐 진행 표시가 보인 뒤 결과가 나온다. 두 번째 글자부터는 즉시다

90. 액티비티 바에 **설정** 뷰가 다섯 번째로 있다. 처음 펼치면 잠깐 비어 있다가(색인 생성) 플러그인 목록이 채워진다. 두 번째부터는 즉시다
91. 설정 항목에 `absenceCriteria  configselect`처럼 설정 종류가 부제로 붙는다. 클릭 → `settings.php`의 그 `admin_setting_*` 줄로 이동
92. 뷰 제목줄마다 **돋보기 버튼**이 있다. 테이블 뷰의 버튼을 누르면 안내문이 `테이블에서 이름·한국어 값으로 찾습니다`이고, 테이블만 나온다(문자열이 섞이지 않는다)
93. 설정 뷰의 돋보기를 **설정 뷰를 한 번도 펼치지 않은 상태에서** 눌러도 결과가 나온다(버튼이 색인을 만든다)
94. 설정 뷰를 한 번도 열지 않은 세션에서 `CSMS Code: 찾기`로 전체 검색하면 설정은 **안 나온다**. 설정 뷰를 한 번 연 뒤 다시 검색하면 나온다
95. 명령 팔레트에 `CSMS Code:`를 치면 `찾기`·`사용처 색인 다시 만들기`·`업데이트 확인` 셋만 보인다(뷰 버튼용 다섯은 숨겨져 있다)
