...
이 실험에서 짧은 캡처만 보관한 경우에도 8.0 MB가 남았는데, 이는 프로브 자신의 지역 변수가 마지막 텍스트를
붙잡은 것이고 모듈 스코프 `RegExp`의 보유가 아니다. 후속 측정이 이를 확인했다 — 정규식의 대상 문자열을
매 파일마다 끊어도(`exec('')`) 보유량이 23.2 MB로 변하지 않았다. 정규식 객체는 대상을 붙잡지 않는다.
...
`phpdocVars`도 다른 팩트와 같은 조각이 된다 — 패턴은 `(comment) @c`이고, 그 텍스트에 정규식을 돌린다.
전체 노드 재귀 워크(1.15 ms)가 사라지고, 통합 쿼리에 패턴 하나가 느는 비용은 무시할 수 있다.

문서 전체에 정규식을 돌리는 대안은 쓰지 않는다. 현행은 주석 노드 안에서만 `@var`를 찾으므로
문자열 리터럴에 든 `@var Type $x`를 잡지 않고, 팩트의 `index`가 매치 위치가 아니라 **주석 노드의 시작**이다.
조각으로 만들면 두 성질이 그대로 보존되어 팩트 diff(§7.2)에 차이가 남지 않는다.
...
- 모듈 스코프 `RegExp`는 건드리지 않는다 — 대상 문자열을 붙잡지 않는 것이 측정으로 확인됐다.
  파일마다 정규식을 새로 만드는 변형은 빌드가 5.4 s → 9.0 s로 느려지고 보유량 이득은 측정 편차 안이다.
...
|---|---|---|
| 현재 | 131 MB | 6.3 s |
| 캡처 12곳 평탄화 | 25.5 MB | 5.9 s |
| 평탄화 + 풀(중복 제거) | **22.8 MB** | 5.4 s |

목표는 **23 MB 이하**다. 105 MB는 사본을 만드는 것만으로 빠지고, 풀은 그 위에서 2.7 MB를 더 깎으면서
raw 문자열이 저장 구조에 들어갈 수 없게 만든다 — 이 구조적 강제가 풀을 쓰는 주된 이유이고, 2.7 MB는 부수 효과다.
항목을 수치 배열로 바꾸는 컬럼형 저장은 여기서 하지 않는다: 추가 이득이 약 7 MB로 추정되는 반면
증분 갱신과 게시 목록 재구성이 타입 배열 위에서 돌아야 해 복잡도가 그에 비해 크다. §7.3 게이트가 실제 수치를 고정한다.
