...
2. **두 번째부터 즉시**: 색인을 디스크에 저장하고, 다음에는 0.1초(실측)에 로드해 바로 쓸 수 있게 한다. 로드 직후 백그라운드로 검증(빠른 순회 + 병렬 stat)해 바뀐 파일만 다시 읽는다.
...
**순회** — `listSourceFiles`:
- `realpath`는 **심볼릭 링크로 진입할 때만** 호출한다(루트 진입 1회 포함). 다만 dedupe 집합에는 **모든 디렉터리**가 들어가야 한다 — 일반 하위 디렉터리의 실경로는 부모 실경로에 이름을 붙여 만든다(syscall 없음). 링크로 들어간 디렉터리만 기록하면 실경로와 링크로 두 번 도달하는 디렉터리를 두 번 훑어 사용처 개수가 부푼다(리뷰에서 재현됨).
...
```
- `ensureUsageIndex`는 빌드를 먼저 시작하고 **0.4초 안에 끝나지 않을 때만** 진행률 알림을 연다 — 캐시 히트(0.1초)는 조용히 지나가고, 캐시 여부를 프레젠테이션이 알 필요가 없다.
- 캐시 히트 경로는 try/catch로 감싼다 — 못 믿을 스냅샷 하나로 세션 내내 기능이 죽지 않게, 실패하면 전체 스캔으로 내려간다.
- `revalidateFromRoot`는 순회·stat이 한 건이라도 실패하면 **삭제를 반영하지 않는다**(침묵으로 삼킨 오류를 삭제로 오해해 빈 캐시를 굽지 않기 위해).
- 저장·삭제 증분 뒤에는 캐시 전용 디바운서(10초)로 다시 쓴다. 쓰기는 직렬화하고 임시 파일 이름에 순번을 붙인다(겹친 쓰기가 서로를 덮지 않게).
- mtime+size 판정의 사각을 위해 명령 `csmscode.rebuildUsageIndex`("사용처 색인 다시 만들기")를 둔다.
